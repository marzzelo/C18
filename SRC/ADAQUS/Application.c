//+------------------------------------------------------------------+
//|       $Workfile:: Application.c              $ $Revision:: 80    $
//+------------------------------------------------------------------+
//|         $Author:: valdez                                         $
//|        $Modtime:: 18/12/13 15:22                                 $
//|        $Archive:: /@C18/SRC/ADAQUS/Application.c                 $
//+------------------------------------------------------------------+
...
void ftos(float fd, char *s, uint16_t MULTIPLIER);
			...
			*/
			...
			{				
				...
				case CMD_SETUP: {
					...
					uint8_t from = atoi(parms[2]);
					uint8_t to = atoi(parms[3]);
					
					uint8_t nch = to - from + 1;
					uint16_t tsw = dtsa * nch + 100; // se agregan 100ms al final del barrido
					
					DAQ_setup(&daq, dtsa, tsw, 1, from, to);
				...
				default: {
					...
					if (isdigit(parms[0][0])) {
						c = atoi(parms[0]);						
						if (c < 32) {
							DAQ_outputAddress(&daq, c);
							printf(STR_ADDRESS_SET, c);	
							prompt();
							break;
						}	
					}	
...
static void App_ctor(Application *me) {
    ...
    ***************************************/	
	...
	me->_state = ST_AUTO;  		// estado inicial
}//
