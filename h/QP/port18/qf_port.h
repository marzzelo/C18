...
#define QF_INT_KEY_TYPE         unsigned char
#define QF_INT_LOCK(key_)       do { \
    (key_) = (INTCON & 0b11000000); \
    INTCON &= 0b00111111; \
} while (0)
#define QF_INT_UNLOCK(key_)     (INTCON |= (key_))
...
*
* NOTE02:
* This QP port assumes that interrupt priorities are allowed (IPEN=1) and
* that high-priority interrupts can nest on low-priority interrupts.
* In this mode, the bit INTCON<7> is interpreted as GIEH (high-priority
* interrupt enable) and INTCON<6> is interpreted as GIEL (low-priority
* interrupt enable). The interrupt locking policy saves the current interrupt
* status (the bits GIEH and GIEL) and clears both GIEH and GIEL bits.
* The unlocking policy restores the saved state of the GIEH and GIEL bits.
* In particular, this policy will never enable interrupts inside high-level
* ISRs, and will enable only the high-level interrupts inside the
* low-level ISRs. You should not enable interrupts explicitly inside ISRs.
*
